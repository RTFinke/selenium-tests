...
FOLDERS = [PHOTOS / ('New folder' if n == 1 else f'New folder ({n})') for n in range(1, 10)]
random.SystemRandom().shuffle(FOLDERS)
assert len(FOLDERS) == 9 and all(p.is_dir() for p in FOLDERS)
...
page.insert_text(subtitle['origin'], 'Ubrania ze skaremsafety na zdj\u0119ciach klient\u00f3w.', fontname='Subtitle', fontfile=r'C:\Windows\Fonts\arialbd.ttf', fontsize=13, color=(0.611765, 0.639216, 0.686275))
...
for number in range(1, 10):
    ...
    for column, info in enumerate(images, 1):
        ...
        contain = column == 2 and FOLDERS[number-1].name != 'New folder (5)'
...
target = OUT / 'Siz3r_Wyniki_Skaremsafety_bhp1_9_zestawow.pdf'
result.save(target, garbage=4, deflate=True)
result.close()

check = fitz.open(target)
assert len(check) == 11
assert 'Ubrania ze skaremsafety na zdj\u0119ciach klient\u00f3w.' in check[0].get_text().replace('\u00a0', ' ')
for number in range(1, 10):
    page = check[number]
...
print(f'OK: {len(check)} pages, 27 photographs in order, subtitle and labels verified. {target}')
